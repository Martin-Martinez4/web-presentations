...
const uls = document.querySelectorAll('ul.bullets__classic')
...
for(let ul = 0; ul < uls.length; ul++){
  ...
  el.addEventListener("click", (e) => {
    ...
    for(let i = 0; i < lis.length; i++){
      if(lis[i].classList.contains("is-visible")){
        continue;
      }else{
        lis[i].classList.toggle("is-visible");
        return;
      }
  ...
  for(let i = 0; i < lis.length; i++){
    lis[i].classList.add("fade-in-element")
  }
